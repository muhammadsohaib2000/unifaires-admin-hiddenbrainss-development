...
const AssociatePaymentType = () => {
  ...
  const fetchPaymentTypes = async () => {
    try {
      const res = await axiosInstance.get("/associate-payment-type");
  ...
  const handleCreate = async () => {
    ...
    try {
      setLoading(true);
      const res = await axiosInstance.post("/associate-payment-type", {
        ...postData,
        currency: currency,
      });
      if (res.status) {
  ...
  const handleDeletePaymentType = async () => {
    try {
      ...
      const res = await axiosInstance.delete(
        `/associate-payment-type/${paymentId}`
      );
...
export default AssociatePaymentType;
