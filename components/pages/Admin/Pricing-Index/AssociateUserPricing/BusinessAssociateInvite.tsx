...
import { Button, Form, Input, Spin, Table, Typography, message } from "antd";
...
const BusinessAssociateInvite = () => {
  ...
  const fetchBusinessDiscount = () => {
    axiosInstance
      .get("/associate-business-pricing")
      .then((res) => {
  ...
  const handleSaveClick = async (id: any) => {
    ...
    await axiosInstance
      .post("/associate-business-pricing", {
        businessId: id,
  ...
  const handleUpdatePricing = async (id: any) => {
    ...
    setLoading(true);
    await axiosInstance
      .put(`/associate-business-pricing/${discountId}`, {
        discount: editedText[id],
      })
      .then((res) => {
...
export default BusinessAssociateInvite;
