...
const FundingPaymentType = () => {
  ...
  const fetchPaymentTypes = async () => {
    try {
      ...
      console.log("here is the error for fundingpaymenttype", error);
  ...
  const handleCreate = async () => {
    ...
    try {
      setLoading(true);
      const res = await axiosInstance.post("/funding-payment-type", {
        ...postData,
        currency: currency,
      });
      if (res.status) {
  ...
  const handleDeletePaymentType = async () => {
    try {
      ...
      const res = await axiosInstance.delete(
        `/funding-payment-type/${paymentId}`
      );
...
export default FundingPaymentType;
