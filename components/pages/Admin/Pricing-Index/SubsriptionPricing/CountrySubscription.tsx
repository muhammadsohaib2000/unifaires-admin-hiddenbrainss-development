...
const CountryJobPosting = () => {
  ...
  const fetchCountryDiscount = () => {
    axiosInstance
      .get("/subscription-country-pricing")
      .then((res) => {
        console.log(res);
  ...
  const handleSaveClick = async (name: string) => {
    ...
    await axiosInstance
      .post("/subscription-country-pricing", {
        country: name,
  ...
  const handleUpdatePricing = async (name: string) => {
    ...
    setLoading(true);
    await axiosInstance
      .put(`/job-country-pricing/${discountId}`, {
        discount: editedText[name],
      })
      .then((res) => {
...
export default CountryJobPosting;
