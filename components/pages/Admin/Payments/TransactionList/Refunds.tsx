...
const RefundsTransactionList = () => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [refundedTransactionList, setRefundedTransactionList] = useState<any>();
  ...
  const fetchRefunds = async (page: any) => {
    try {
      setLoading(true);
      const queryParams = {
        page,
      ...
      const res = await axiosInstance.get(`/refund${query}`);
      ...
      if (res.status) {
        ...
        setRefundedTransactionList(resData.transactions);
      }
    } catch (error) {
    ...
    <Fragment>
      ...
      <div className="bg-white rounded-lg pb-3 border overflow-hidden">
        <Spin spinning={loading} size="large">
          <Table
            columns={columns}
            dataSource={refundedTransactionList}
...
export default RefundsTransactionList;
